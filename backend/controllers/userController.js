...
export const login = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!email || !password || !role) {
    return next(new ErrorHandler("Please fill whole form!", 400));
  }
  const user = await User.findOne({ email }).select("+password"); //get passwod
  if (!user) {
  ...
  sendToken(user, 200, "User logged in ", res);
});
