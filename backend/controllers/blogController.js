...
export const blogPost = catchAsyncErrors(async (req, res, next) => {
  console.log("Oh no");
  const { mainImageLink } = req.body;
  const { paraOneImageLink } = req.body;
  const { paraTwoImageLink } = req.body;
  const { paraThreeImageLink } = req.body;
  let fileurl, fileurl1, fileurl2, fileurl3;
  if (mainImageLink) fileurl = await cloudinary.uploader.upload(mainImageLink);
  if (paraOneImageLink)
    fileurl1 = await cloudinary.uploader.upload(paraOneImageLink);
  console.log("Oh no2");
  if (paraTwoImageLink)
    fileurl2 = await cloudinary.uploader.upload(paraTwoImageLink);
  if (paraThreeImageLink)
    fileurl3 = await cloudinary.uploader.upload(paraThreeImageLink);
  // console.log(fileurl);
  ...
  console.log("Oh no");
  let files = req.files || {};
  let paraOneImage = files.paraOneImage || null;
  let mainImage = files.mainImage || null;
  let paraTwoImage = files.paraTwoImage || null;
  let paraThreeImage = files.paraThreeImage || null;
  console.log("Oh no");
  if (!mainImage && !fileurl) {
    return next(new ErrorHandler("Blog Main Image Is Mandatory!", 400));
  }
  const allowedFormats = ["image/png", "image/jpeg", "image/webp", "image/jpg"];
  if (
    (mainImage && !fileurl && !allowedFormats.includes(mainImage.mimetype)) ||
    (paraOneImage &&
      !fileurl1 &&
      !allowedFormats.includes(paraOneImage.mimetype)) ||
    (paraTwoImage &&
      !fileurl2 &&
      !allowedFormats.includes(paraTwoImage.mimetype)) ||
    (paraThreeImage &&
      !fileurl3 &&
  ...
  const uploadPromises = [
    ...
    paraOneImage && !fileurl1
      ? cloudinary.uploader.upload(paraOneImage.tempFilePath)
      : Promise.resolve(null),
    paraTwoImage && !fileurl2
      ? cloudinary.uploader.upload(paraTwoImage.tempFilePath)
      : Promise.resolve(null),
    paraThreeImage && !fileurl3
      ? cloudinary.uploader.upload(paraThreeImage.tempFilePath)
      : Promise.resolve(null),
  ];

  let [mainImageRes, paraOneImageRes, paraTwoImageRes, paraThreeImageRes] =
    await Promise.all(uploadPromises);

  if (
    (mainImage && !fileurl && (!mainImageRes || mainImageRes.error)) ||
    (paraOneImage &&
      !fileurl1 &&
      (!paraOneImageRes || paraOneImageRes.error)) ||
    (paraTwoImage &&
      !fileurl2 &&
      (!paraTwoImageRes || paraTwoImageRes.error)) ||
    (paraThreeImage &&
      !fileurl3 &&
      (!paraThreeImageRes || paraThreeImageRes.error))
  ) {
    return next(
  ...
  console.log("Oh no3");
  if (fileurl) mainImageRes = fileurl;
  if (fileurl1) paraOneImageRes = fileurl1;
  if (fileurl2) paraTwoImageRes = fileurl2;
  if (fileurl3) paraThreeImageRes = fileurl3;
  const blogData = {
    title,
    intro,
    paraOneDescription,
...
export const updateBlog = catchAsyncErrors(async (req, res, next) => {
  ...
  console.log(newBlogData.category);
  if (req.files) {
